    ...
    // Access the stats global from main.cpp (Needed for Objective 4)
    ...
    // --- CONSTANTS ---
    const float MIN_STOP_DISTANCE = 2.0f;
    ...
    struct ComparePrioNode {
        bool operator()(const PrioNode& a, const PrioNode& b) { return a.first > b.first; }
    };
    ...
    std::vector<glm::vec3> FindPathDijkstra(const LaneGraph& lanegraph, const glm::vec3& start, const glm::vec3& goal) {
        ...
        current = goal;
        while (current != start) { path.push_back(current); current = came_from[current]; }
    ...
    void UpdateDotIDM(Dot& dot, const Dot* leader, float distToLeader, float deltaTime)
    {
        ...
        // --- OBJECTIVE 4: STATS TRACKING ---
        // Track total time alive
        dot.totalTimeAlive += deltaTime;

        // Track Waiting Time (Delay)
        // If speed is less than 1.0 m/s (approx 3.6 km/h), considering it "stopped/waiting"
        if (dot.v < 1.0f) {
            dot.currentWaitTime += deltaTime;
        }
        // -----------------------------------
        ...
        if (redLightAhead && distToRedLight < gap_car) {
            ...
            effective_v_target = 0.0f;
        }

        if (effective_gap < 0.1f) effective_gap = 0.1f;
        ...
        // --- OBJECTIVE 4: TRIP COMPLETION CHECK ---
        if (dot.segment + 1 >= path.size()) {
            // Car has finished the trip! Record data.
            currentStats.totalTripsFinished++;
            currentStats.totalTravelTime += dot.totalTimeAlive;
            currentStats.totalWaitTime += dot.currentWaitTime;
