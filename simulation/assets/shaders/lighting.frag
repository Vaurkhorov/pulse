...
// NEW: Toggle for debug rendering
uniform bool useFlatColor; 
uniform vec3 flatColor;    // The color to use when flat shading is on

void main()
{
    // --- MODE 1: DEBUG / FLAT COLOR ---
    if (useFlatColor) {
        FragColor = vec4(flatColor, 1.0);
        return; // Skip lighting calculations
    }

    // --- MODE 2: STANDARD LIGHTING (Your existing code) ---
    ...
    // "Paint" the lane markings
    vec3 centerLineColor = vec3(0.9, 0.8, 0.2);
    vec3 laneLineColor = vec3(1.0, 1.0, 1.0);
    float centerLineWidth = 0.015;
    float laneLineWidth = 0.01;
